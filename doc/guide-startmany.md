...
Copy the masternode private key and correspondig collateral output transaction that holds the 1K Gossipcoin.

The masternode private key may be an existing key from [Option 1](#option1), or a newly generated key from [Option 2](#option2).

*Please note, the masternode priviate key is not the same as a wallet private key. Never put your wallet private key in the masternode.conf file. That is equivalent to putting your 10,000 Gossipcoin on the remote server and defeats the purpose of a hot/cold setup.*
...
If you are using a masternode.conf file you no longer need the gossipcoin.conf file. The exception is if you need custom settings (thanks oblox).

## Update gossipcoin.conf on server

If you generated a new masternode private key, you will need to update the remote gossipcoin.conf files.

Shut down the daemon and then edit the file.

```sudo nano .gossipcoin/gossipcoin.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the masternodeprivkey value in your remote gossipcoin.conf file.
