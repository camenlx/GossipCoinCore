...
- [Building Gossipcoin](#building-gossipcoin)
...
Clone the git repositories for gossipcoin and gitian and then checkout the gossipcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the gossipcoin repository
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gossipcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/Gossipcoin
...
./bin/gbuild --commit gossipcoin=${COMMIT} --url gossipcoin=${URL} ../Gossipcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gossipcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gossipcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/gossipcoin-build.assert
```
