...
#include "zgossipcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGOSS ought to be enough for anybody." - Bill Gates, 2017
    ui->zGOSSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGOSSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGOSS->setEnabled(false);
        ui->pushButtonMintzGOSS->setToolTip(tr("zGOSS is currently disabled due to maintenance."));

        ui->pushButtonSpendzGOSS->setEnabled(false);
        ui->pushButtonSpendzGOSS->setToolTip(tr("zGOSS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGOSSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGOSS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGOSS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGOSS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGOSS in ") +
...
void PrivacyDialog::on_pushButtonSpendzGOSS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGOSS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGOSS
        sendzGOSS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGOSS
    sendzGOSS();
}

void PrivacyDialog::on_pushButtonZGossipcoinControl_clicked()
{
    ZGossipcoinControlDialog* zGOSSControl = new ZGossipcoinControlDialog(this);
    zGOSSControl->setModel(walletModel);
    zGOSSControl->exec();
}

void PrivacyDialog::setZGossipcoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGOSSSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGOSS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Gossipcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGOSSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGOSS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GOSSIPCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGOSS</b>";
    ...
    // use mints from zGOSS selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGossipcoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGossipcoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zgossipcoin selector in case it was used
    ZGossipcoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGOSS Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Gossipcoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGOSS Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGOSS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGOSS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGOSS "));
    ui->labelzGOSSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGOSSSyncStatus->setVisible(fShow);
}
