...
#include "ui_zgossipcoincontroldialog.h"
...
std::list<std::string> ZGossipcoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGossipcoinControlDialog::listMints;

ZGossipcoinControlDialog::ZGossipcoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGossipcoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGossipcoinControlDialog::~ZGossipcoinControlDialog()
{
    delete ui;
}

void ZGossipcoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGossipcoinControlDialog::updateList()
{
...
void ZGossipcoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGossipcoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGossipcoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGossipcoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGossipcoinControlDialog::GetSelectedMints()
{
...
void ZGossipcoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
